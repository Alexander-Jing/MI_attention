...
%ChoiceTrial = ChoiceTrial.session;
 ChoiceTrial = [1,2,1,2,1,2,1,2,1,2,1,2,1,2,1,2,1,2,1,2,1,2,1,2,1,2,1,2];  % 临时使用
%% �?始实验，离线采集
Timer = 0;
TrialData = [];
...
%            if length(scores) > 2
%                scores_v = scores(end) - scores(end-1);
%                if scores_v > 0
%                     score_thre1 = score_thre1 + 0.1 * scores_v;
%                else
%                    score_thre1 = score_thre1 + 0.75 * scores_v;
%                end
