...
data_x = load('DataX.mat');
%disp(data_x.DataX{1,1});
%% 数据传输测试
dataBytes = getByteStreamFromArray(data_x);  % 将数据转化为字节数组，这里的数据是matlab的double类型的，总共是8 bytes每一个数据

config = whos('dataBytes');
...
% send_order = 1.0;  % 发送命令控制，用于控制服务器
% send_data = [send_order; config.size(:); dataBytes(:)];
