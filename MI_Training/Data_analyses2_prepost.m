...
%% 闁稿繑婀圭紞鎴﹀礄閼恒儲娈堕梺顔哄妼閸ㄥ酣鏁嶇仦钘夌樁闁瑰鍓氶幎銈呪枖椤???娴ｇǹ鐏婄紒鎰殔閸ら亶寮??
% 婵犲ǹ鍊栫亸婵嬪礄閼恒儲娈?
function FilteredData = DataFilter(RawData, sample_frequency) 
    FilterOrder = 4;  % 閻犱礁澧介悿鍡欐暜閿???濮橆厽濮㈡繛澶堝灩濞呮帡鎯冮崟顖涒枆闁???
    NotchFilterOrder = 2;  % 閻犱礁澧介悿鍡涙⒔闁垮鐨炬繝濞垮?栫亸婵嬪闯閵娧勭暠闂傚啳鍩栭弳鐔兼晬閸絿绠归梺鎻掑婵炲洭鎮介妸銉ュ冀闁绘顫夐惌渚?寮姘辨暔闂傚啰绮幎銈呪枖閵忕姵鐝ら柨??
    %Wband = [3,50];  % 婵犲ǹ鍊栫亸婵嬪闯閵娿劎绠归弶鍫濈秺濞撳墎鎲版担绋挎闁兼澘鍟板ù澶愬礂瀹曞洦鐣遍柡鍌氭川鐏忕偞娼诲☉婊庢斀濞ｅ浂鍠楅弫濂告晬瀹?鍐闂佹彃鑻顒勬嚀閸愌冨皟闁哄嫮鍠庣粭?濠殿喗鍔楀▓鎴犳媼閻戞ɑ鐎☉鎿冨幘濞堟垵顭ㄩ妶鍡楃毦闁革絻鍔忛鏇犵磾?
    Wband = [8,12];
    Wband_notch = [49,51];
    FilterType = 'bandpass';
    FilterTypeNotch = 'stop';  % matlab闁汇劌鍨簎tter闁告垼濮ら弳鐔兼煂瀹?鍕〃闁挎稑鐭侀鏇犵磾?'stop'濞村吋淇洪崵婊堝礉閵婎煈鍟庣紓鍐惧枟閸??2闂傚啳鍩栭幎銈呪枖閵忕姵鐝?

    % 濞达綀娉曢弫銈夋⒔闁垮鐨炬繝濞垮?栫亸婵嬪闯閵娿儱绠甸梻鍕╁?曟导鎰紣閹存繃鐝ㄥ??
    FilteredData = Rsx_ButterFilter(NotchFilterOrder,Wband_notch,sample_frequency,FilterTypeNotch,RawData,size(RawData,1));
    % 濞达綀娉曢弫銈囨暜閿???濮橆厽濮㈡繛澶堝灩濞呮帡宕㈡繝姘彑闁革綆浜滈敍?
    FilteredData = Rsx_ButterFilter(FilterOrder,Wband,sample_frequency,FilterType,FilteredData,size(FilteredData,1)); 
end

% 閻犱緤绱曢悾濠氬礆閹烘梻宕堕柣銊ュ閸ら亶寮??
function [windows_per_session, DataSamplePre] = WindowsDataPre(RawData, WindowLength, SlideWindowLength)
    data_points_per_session = size(RawData,2);  % 婵絽绻嬬粩瀛樼▔閻氱暴ssion闁汇劌瀚弳鐔煎箲椤??閸??
    % seconds_per_session  = size(EIRawData,2)/sample_frequency;  % 婵絽绻嬬粩瀛樼▔閻氱暴ssion闁汇劌瀚鍌炴⒒閹绢喗姣愰幖??

    windows_per_session = (data_points_per_session - WindowLength) / SlideWindowLength + 1;  % session婵犲﹥鍨归悰銉╁触鎼达絾鐣辩紒鎰殕閺嗙喖鏌??
...
function [DataSample, LabelWindows] = DataWindows(DataSamplePre, FilteredData, channels, class_index, windows_per_session, SlideWindowLength, WindowLength, sample_frequency)
    ...
    % channels = [3,8,27,28,30,31,32,33]-1;  % 缁绢収鍠栭悾楣冨礈瀹ュ鏉洪柛娆忓濞堟垿鏌呭鏈靛闁挎稑鐬奸弫杈ㄧ鎼淬値鍞剁憸鐗堟礈濞堟垶绌遍垾鍐插▏CH-1闁哄嫮鏌杛igger闁挎稑鏈晶宥嗙閵夛箑顣查柡鍫濐槺濞堟垹妲愰姀鐘电┛闁告垵绻愰獮?1
    LabelWindows = [];
    % scores = [];  % 闁活潿鍔嬬花顒??娑櫭崑宄碿ores闁告帒妫欓弳鐔兼儍閸曨剚娈剁紓??

    % 闁汇垻鍠愰崹姘跺礆閹烘梻宕堕柣銊ュ閺嗙喖骞??
    for i = 1:windows_per_session
        PointStart = (i-1)*SlideWindowLength;  % 闁革负鍔嶉弳鐔煎箲椤旀槒鍘痪顓у枛閻ｅ墽鎸у畡閭︽綏闁???
        DataSamplePre{1, i} = FilteredData(channels, PointStart + 1:PointStart + WindowLength );  % 闁汇垻鍠愰崹姘跺礆閹烘梻宕堕柣銊ュ閸樻挾绮??
        LabelWindows = [LabelWindows; class_index];  % 闁汇垻鍠愰崹姘辨啑閸椻暆bel闁汇劌瀚弳鐔煎箲?
    end
