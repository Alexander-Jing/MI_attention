...
% 运动想象基本参数设置
subject_name = 'Jyt_test_0129_online_debug';  % 被试姓名
sub_offline_collection_folder = 'Jyt_test_0129_offline_debug_20240129_224307245_data';  % 被试的离线采集数据
subject_name_offline =  'Jyt_test_0129_offline_debug';  % 离线收集数据时候的被试名称
...
TrialNum = 30;  % 每一个session里面的trial的数量
...
RestTimeLenBaseline = 5 + session_idx;  % 休息时间随着session的数量增加
...
%         end   
   ...
   %% 休息阶段，确定下一个动作
    ...
    if Timer == 10 && Trials(AllTrial)==0 && clsControl == 0 %结束休息，准备下一个
        % 存储相关的EI指标和mu节律能量的数据
        SaveMIEngageTrials(EI_indices, mu_powers, mu_suppressions, subject_name, foldername, config_data, EI_index_scores, resultsMI, FES_flags);
        %计时器清0
        Timer = 0;  % 计时器清0
        % 每一个trial的数值还原
        scores = [];  % 分数值还原
