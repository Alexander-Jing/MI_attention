...
% 运动想象基本参数设置
subject_name = 'Jyt_test_0306_post_control_1';  % 被试姓名
TrialNum = 20;  % 设置采集的数量
%TrialNum = 3*3;
MotorClasses = 2;  % 运动想象的种类的数量的设置，对比实验的任务是[1,2]，和readme.txt里面的对应
...
randomindex = [];   
if MotorClasses > 1                                                       % 初始化trials的集合
    ...
    RandomTrial = repmat(MotorClassMI, TrialNum);   % 如果是单任务的话，我们直接设置任务
end
%% 实验数据采集存储设置
% 设置相关参数
...
EI_index_scores_trialmean = [];  % 用于存储每一个trial里面的EI指标均值
mu_suppressions_trialmean = [];  % 用于存储每一个trial里面的mu_suppressions指标均值
%% 开始实验，离线采集
Timer = 0;
TrialData = [];
while(AllTrial < (TrialNum+1))
    ...
    if Timer==2
        Trigger = RandomTrial(AllTrial);  % 切换到画面，但是不播放动作的AO动画（Idle, MI1, MI2）
